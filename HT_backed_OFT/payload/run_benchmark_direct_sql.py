...
"""
run_benchmark_direct_sql.py — 8-thread Direct SQL benchmark for SPCS ML Job.

Submitted via snowflake.ml.jobs.submit_directory.
Runs inside SPCS container with env vars injected by submit_job_direct_sql.py.

Uses cursor.execute() against the Hybrid Table $ONLINE table directly,
bypassing the Snowpark SDK overhead. This is the fastest path from SPCS
since the SQL REST API (/api/v2/statements) is blocked by network policy
for SPCS egress IPs.

Config: 8 threads, 600s warmup, 300s measurement.
Each thread gets its own cursor on the shared session connection.
"""
...
FV_VER     = "v1"
ONLINE_TABLE = f'{DATABASE}.{FS_SCHEMA}."{FV_NAME}${FV_VER}$ONLINE"'
N_KEYS     = 100_000
N_THREADS  = 8
WARMUP_SECONDS  = 600
MEASURE_SECONDS = 300
...
ENV_TAG = f"SPCS_DSQL_{N_THREADS}T"
...
try:
    ...
    print(f"Auth: active SPCS session  host={_spcs_host}")
except Exception:
    print(f"Auth: PAT fallback  host={_spcs_host or '(derived from account)'}")
...
paramstyle = conn.cursor().execute("SELECT 1").description and "qmark"
print(f"Connection paramstyle: qmark (SPCS)")

feature_cols = ", ".join([f'"FEATURE_{i:02d}"' for i in range(1, NUM_FEATURES + 1)])
select_cols = f'"CUSTOMER_ID", {feature_cols}, "UPDATED_AT"'
_query = f'SELECT {select_cols} FROM {ONLINE_TABLE} WHERE "CUSTOMER_ID" = ?'
print(f"Online table: {ONLINE_TABLE}")
print(f"Query: {_query}")

print("\nSanity check...")
cur = conn.cursor()
cur.execute(_query, ("CUST_0000000000",))
rows = cur.fetchall()
assert len(rows) > 0, "Sanity check failed: no rows returned"
print(f"Sanity check: OK ({len(rows)} row, {len(rows[0])} cols)")
cur.close()

cursors = [conn.cursor() for _ in range(N_THREADS)]
print(f"Created {N_THREADS} cursors on shared connection")

_perf = time.perf_counter


def worker_fn(thread_id, phase, duration_s):
    cur = cursors[thread_id]
    latencies = []
    count = 0
    start = time.time()
    ...
    while time.time() < deadline:
        ...
        if count % 1000 == 0 and thread_id == 0:
...
print(f"\n{'='*60}")
print(f"  Direct SQL Benchmark — {N_THREADS} threads (multi-cursor)")
...
_json_path = f"/tmp/benchmark_dsql_{_run_id}.json"
with open(_json_path, "w") as _fh:
    json.dump(_payload, _fh)

session.sql(f"PUT file://{_json_path} @{DATABASE}.{SCHEMA}.FS_BENCH_JOB_RESULTS AUTO_COMPRESS=TRUE").collect()
print(f"Raw latency array uploaded to @FS_BENCH_JOB_RESULTS/benchmark_dsql_{_run_id}.json.gz")
