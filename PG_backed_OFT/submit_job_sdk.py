...
"""
submit_job_sdk.py — Submit the SDK-based latency benchmark as a headless ML Job on SPCS.

Runs `fs.read_feature_view(fv, keys=[[key]], store_type=StoreType.ONLINE).collect()`
against the Postgres Online Service via the Snowpark Feature Store SDK.

Usage:
    export SNOWFLAKE_PAT='<your-pat>'
    export SNOWFLAKE_USER='<your-username>'          # e.g. GHAZALEH
    python submit_job_sdk.py [--wait] [--logs]

Flags:
    --wait   Block until the job finishes and print its exit status.
    --logs   Stream job logs to stdout after completion (implies --wait).
"""
...
def main():
    parser = argparse.ArgumentParser(description="Submit Postgres SDK latency benchmark as a headless ML Job")
    ...
    print(f"  python submit_job_sdk.py --logs")
